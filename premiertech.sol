...
contract JetPack is ERC721URIStorage, AccessControl, ERC721Enumerable, Ownable {
    ...
    uint256 private _MINT_NFT_FEE;
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    uint256 public MAXIMUM_TOTAL_SUPPLY;
    uint256 private _PRE_SALE_TIME_START;

    uint256 private _PRE_SALE_TIME_END;

    uint256 private _PUBLIC_SALE_TIME_START;
    ...
    //to see if the users have already minted their NFT
    ...
    constructor() ERC721("Jadu Jetpack", "JETPACK") {
        ...
        //varuavles to change
        _MINT_NFT_FEE = 110000000000000000;
        _PRE_SALE_TIME_START = 1631815200; // 6 PM UTC Time
        _PRE_SALE_TIME_END = 1631826000; // 9 PM UTC Time
        _PUBLIC_SALE_TIME_START = 1631827800; // 9:30 PM UTC TIME
    ...
    //usres that are whitelisted will be allowed to mint 1 NFT
    function preSaleMint(address _mintTo, string memory _tokenURI) external payable returns (bool) {
        ...
        require(block.timestamp >= _PRE_SALE_TIME_START && block.timestamp <= _PRE_SALE_TIME_END, "JETPACK: This is not pre sale time for minting");
        require(totalSupply() + 1 <= MAXIMUM_TOTAL_SUPPLY, "JETPACK: Maximum Supply Minted");
        require(msg.value == _MINT_NFT_FEE, "JETPACK: Invalid Minting Fee");
    ...
    //users will only be able to mint one NFT in public sale
    function publicSaleMint(address _mintTo, string memory _tokenURI) external payable returns (bool) {
        require(block.timestamp >= _PUBLIC_SALE_TIME_START, "JETPACK: Public Sale not yet started");
        require(!publicSaleAddressesMint[msg.sender], "JETPACK, You have already minted public sale token");
        require(totalSupply() + 1 <= MAXIMUM_TOTAL_SUPPLY, "JETPACK: Maximum Supply Minted");
        require(msg.value == _MINT_NFT_FEE, "JETPACK: Invalid Minting Fee");
        
        //payable(JETPACK_ADMIN_WALLET).transfer(msg.value); this method is deprecated changed it for the following
        _withdraw();
        ...
        _mint(_mintTo, newItemId);
        _setTokenURI(newItemId, _tokenURI );
        publicSaleAddressesMint[msg.sender] = true;
        
        return true;
    }
    
    //sends funds to owners account
    ...
    //FUNCTIONS TO VIEW WHAT PART OF SALE WE ARE IN AND TO SEE MINT FEE
    function MINT_NFT_FEE() public view returns(uint256) {
        return _MINT_NFT_FEE;
    }
    
    function inPreSale() public view returns(bool) {
        return block.timestamp >= _PRE_SALE_TIME_START && block.timestamp <= _PRE_SALE_TIME_END;
    }
    
    function inPublicSale() public view returns(bool) {
        return block.timestamp >= _PUBLIC_SALE_TIME_START;
    }
